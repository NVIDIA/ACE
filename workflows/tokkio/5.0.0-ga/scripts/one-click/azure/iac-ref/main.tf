...
module "app_gateway" {
  ...
  probes = [
    for port in sort(keys(each.value["ports"])) : {
      ...
      host     = local.hostname_map[contains(keys(local.hostname_map), port) ? port : "default"]
      match = {
        status_code = ["200-399"]
      }
  ...
  backend_http_settings = [
    for port in sort(keys(each.value["ports"])) : {
      ...
      request_timeout = each.value["ports"][port]["request_timeout"]
    }
  ]
  ...
  http_listeners = [
    for port in sort(keys(each.value["ports"])) : {
      ...
      host_names                     = [local.hostname_map[contains(keys(local.hostname_map), port) ? port : "default"]]
...
module "ace_configurator_dns" {
  ...
  source            = "../modules/azure/dns-a-record"
  name              = local.ace_configurator_sub_domain
  base_domain       = local.base_domain
  ttl               = 3600
  azure_resource_id = null
  ip_addresses      = [module.app_gateway[each.key].public_ip[each.value["name"]]]
