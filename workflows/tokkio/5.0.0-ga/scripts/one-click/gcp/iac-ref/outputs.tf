output "info" {
  value = {
    ...
    #       # var.clusters[cluster].private_instance ? module.load_balancer[cluster].addresses[port_name]["address"] : module.master[cluster].public_ip,
    #       var.clusters[cluster].private_instance ? "80" : var.clusters[cluster].ports[port_name].port,
      ...
      cluster => merge(
        ...
        },
        {
          ...
          format(
            ...
            local.ssh_user,
            var.clusters[cluster].private_instance ? module.node[format("%s-%s", cluster, node)].private_ip : module.node[format("%s-%s", cluster, node)].public_ip
          )
        }
      )
    }
  ...
  value = {
    ...
    clusters = {
      ...
      cluster => {
        ...
        endpoint = {
          api_endpoint =  var.clusters[cluster].private_instance ? local.api_domain_dns : null
        }
        ui_storage = {
          ui_bucket_id = var.clusters[cluster].private_instance ? module.ui_bucket[cluster].name : null
        }
        platform = {
          ace_configurator_endpoint = local.ace_configurator_domain
