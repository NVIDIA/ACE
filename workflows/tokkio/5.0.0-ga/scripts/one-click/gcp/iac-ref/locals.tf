...
locals {
  ...
  ace_configurator_backend_config = {
    name                  = format("%s-ace-cfg", local.name)
    port_name             = "ace-cfg-port"
    locality_lb_policy    = "ROUND_ROBIN"
    load_balancing_scheme = "EXTERNAL_MANAGED"
    timeout_sec           = 600
    access_policy = {
          ...
          matches = [
            {
              ...
              configs = [
                {
                  src_ip_ranges = local.dev_access_cidrs
                }
              ]
            }
          ]
  ...
  ace_configurator_domain_dns = format("%s.%s", local.ace_configurator_sub_domain, local.base_dns_name)
