...
module "ace_configurator_backend" {
  for_each = {
      for k, v in var.clusters : k => {
        name = format("%s-%s-ace-cfg-backend", local.name, k)
      } if v.private_instance
    }
  source                = "../modules/gcp/compute-backend-service"
  name                  = each.value.name
  port_name             = local.ace_configurator_backend_config.port_name
  locality_lb_policy    = local.ace_configurator_backend_config.locality_lb_policy
  load_balancing_scheme = local.ace_configurator_backend_config.load_balancing_scheme
  group                 = module.api_instance_group[each.key].id
  access_policy         = local.ace_configurator_backend_config.access_policy
  http_health_checks    = local.ace_configurator_backend_config.http_health_checks
  timeout_sec           = local.ace_configurator_backend_config.timeout_sec
  }
...
module "api_load_balancer" {
  ...
  host_rules       = concat(
                      local.api_lb_config.host_rules,
                      [{
                        hosts        = [local.ace_configurator_domain]
                        path_matcher = "ace-configurator-matcher"
                      }]
                    )
  path_matchers    = concat(
                      local.api_lb_config.path_matchers,
                      [{
                        name = "ace-configurator-matcher"
                        path_rules = [
                        ]
                        default_service = module.ace_configurator_backend[each.key].id
                      }]
                    )
