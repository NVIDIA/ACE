...
class TokkioNvidiaRAGService(NvidiaRAGService):
    ...
    async def _get_rag_response(self, request_json: dict):
        resp = await self.shared_session.post(f"{self.rag_server_url}/generate", timeout=15.0, json=request_json)
        return resp
    ...
    async def _process_context(self, context: OpenAILLMContext):
        try:
            ...
            logger.error(f"An error occurred in http request to RAG endpoint, Error:  {e!r}")
