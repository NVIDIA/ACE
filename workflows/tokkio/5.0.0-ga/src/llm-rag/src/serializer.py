...
from nvidia_pipecat.frames.custom_view import (
    ...
    StopCustomViewFrame,
    UISelectableOptionPressFrame,
    UIButtonPressFrame,
    UIInterimTextInputFrame,
    UITextInputFrame,
)
    ...
    async def deserialize(self, data: str | bytes) -> Frame | None:
        ...
        if "type" in message:
            match message.get("type"):
                ...
                case "rag_settings":
                    settings = {}
                    if message.get("collection_name", None) is not None:
                        settings["collection_name"] = message.get("collection_name", None)
                    if message.get("use_knowledge_base", None) is not None:
                        settings["use_knowledge_base"] = message.get("use_knowledge_base", None)
                    if message.get("rag_server_url", None) is not None:
                        settings["rag_server_url"] = message.get("rag_server_url", None)
                    return NvidiaRAGSettingsFrame(settings=settings)
                case "button_pressed":
                    if message.get("toggled", None) is not None:
                        return UISelectableOptionPressFrame(component_id=message.get("id", ""), action_id="", toggled=message.get("toggled", False))
                    else:
                        return UIButtonPressFrame(component_id=message.get("id", ""), action_id="")
                case "key_pressed":
                    return UIInterimTextInputFrame(component_id=message.get("id", ""), action_id="", interim_input=message.get("interim_input", ""))
                case "editing_finished":
                    return UITextInputFrame(component_id=message.get("id", ""), action_id="", input=message.get("input", ""), enter_pressed=message.get("enter_pressed", True))
                case _:
                    pass
        return None
