...
DEVNAME="/dev/$(lsblk -bdnJI 8,259 -o NAME,SIZE,SERIAL  | jq -r '.blockdevices |  sort_by(.size) | reverse |map(select(.serial | endswith("data")))|map(.name) | .[0]')"
pvcreate "${DEVNAME}"
lvm_group_name=rp-volume-group
